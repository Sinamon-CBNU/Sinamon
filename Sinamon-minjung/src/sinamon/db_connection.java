...
public class db_connection {
	...
	public db_connection() {
		try {
			...
			System.out.println("데이터베이스 연결 오류(jdbc driver): " + e.getMessage());
		}
	}
	...
	public void input_reservation(String board_name, String time, String place, String memo) {
		try {
			String SQL1 = "INSERT INTO food_reserve_board(board_id, time, location, memo) VALUES(1, " + time + ", " + place + ", " + memo + ");";
			int record_num = st.executeUpdate(SQL1);		//INSERT 쿼리는 executeUpdate를 사용한다. 이것은 업데이트된 건수를 int로 리턴한다.
			System.out.println("업데이트된 건 수: " + record_num);
			
			String SQL2 = "UPDATE " + board_name + " SET is_complete = '진행중' WHERE id = 1;";		//예정->진행중으로 업데이트
			int record_num2 = st.executeUpdate(SQL2);		//INSERT 쿼리는 executeUpdate를 사용한다. 이것은 업데이트된 건수를 int로 리턴한다.
			System.out.println("업데이트된 건 수: " + record_num);
			
		} catch (Exception e) {
			System.out.println("데이터베이스 입력 오류-input_reservation/ Code: " + e.getMessage());
		}
	}
	...
	public void modify_board(String location, String time, String title) {
		try {
			System.out.println("출력");
			String SQL = "";
			SQL = "UPDATE food_board SET time= '" + time + "', title= '" + title + "'  WHERE id = 17;";
			int record_num = st.executeUpdate(SQL);		//INSERT 쿼리는 executeUpdate를 사용한다. 이것은 업데이트된 건수를 int로 리턴한다.
			System.out.println("업데이트된 건 수: " + record_num);
		} catch (Exception e) {
			System.out.println("데이터베이스 입력 오류(수정)/ Code: " + e.getMessage());
		}
	}
